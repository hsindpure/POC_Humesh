...
console.log("absjgdck");
    $(this).parent().closest('.chart-container').addClass('active');
})
...
function updateHeader(tabName) {
    const tabInfo = {
        Summary: { title: 'Summary', subtitle: 'Real-time analytics and insights' },
        Base1: { title: 'Base1', subtitle: 'Sales performance and trends' },
        Base2: { title: 'Base2', subtitle: 'Customer insights and segmentation' },
    };
    
    const info = tabInfo[tabName] || tabInfo.Summary;
...
function toggleFilterPanel() {
    ...
    $('.filter-panel').toggleClass('collapsed', dashboardState.filterPanelCollapsed);
